...
public class PlatformLERPToggleSwitch : ToggleSwitch
{
    ...
    public IEnumerator MoveOut(lerpPlatform platform, int index)
    {
        ...
        while ((((PlayerShadowInteraction.m_CurrentPlayerState != PlayerShadowInteraction.PLAYERSTATE.SHIFTING && !GameController.paused) ? localTime += Time.deltaTime : localTime) < (panStart + moveTime[index]) + GameController.masterTimer - localTime) && pressed)
        {
