...
public abstract class ToggleSwitch : MonoBehaviour {
    ...
    {
        if (other.gameObject.tag == "Player" && PlayerShadowInteraction.m_CurrentPlayerState == PlayerShadowInteraction.PLAYERSTATE.FORM && !GameController.switch_cooldown)
        {
            if(!animating)
            {
    ...
    {
        if (other.gameObject.tag == "Player" && PlayerShadowInteraction.m_CurrentPlayerState == PlayerShadowInteraction.PLAYERSTATE.FORM)
        {
            GameController.CheckInteractToolip(false, false);
        }
    }
    ...
    public IEnumerator ControlTimerSwitchAudio()
    {
        ...
        while(currentTimerDuration < audioStart + timerAudioSource.clip.length)
        {
            ...
            if (GameController.paused || PlayerShadowInteraction.m_CurrentPlayerState == PlayerShadowInteraction.PLAYERSTATE.SHIFTING)
            {
